 ...
 * @author VALENTINA
 */
public class RetirarBolsillo extends javax.swing.JFrame {

    private String numero;
    private Bases bases;
    private Connection conexion;
    /**
     * Creates new form RetirarBolsillo
     */
    public RetirarBolsillo(String numero) {
        initComponents();
        this.numero = numero;
    ...
    private void btnConfirmarConsignaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConfirmarConsignaActionPerformed
        ...
        double saldoRetirar =Double.parseDouble(txtSaldoConsignar.getText());
        double saldoDisponible = obtenerSaldoDisponible(nombreBolsillo);
        
        Bolsillo bolsillo = new Bolsillo();
        boolean datosCorrectos = bolsillo.verificarNombreBolsillo(nombreBolsillo);

        if(datosCorrectos){
            if (validarSaldo(saldoDisponible, saldoRetirar)) {
               retirarDinero(nombreBolsillo, numero, saldoRetirar);
            } else {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(RetirarBolsillo.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                new RetirarBolsillo(numero).setVisible(true);
            }
        });
    }
    
    private boolean validarSaldo(double saldoRetirar, double saldoDisponible) {
       
            if(saldoRetirar > saldoDisponible){
                return saldoRetirar >= 0;
            }else{
            return false;
            }
    ...
    private double obtenerSaldoDisponible(String nombreBolsillo) {
    Connection conexion = bases.conectar();
    try {
        Statement statement = conexion.createStatement();
        String query = "SELECT SaldoDisponible FROM bolsillo WHERE NombreBolsillo = '" + nombreBolsillo + "'";
        ResultSet resultSet = statement.executeQuery(query);

        if (resultSet.next()) {
            return resultSet.getDouble("SaldoDisponible");
        } else {
    ...
    private void retirarDinero(String nombreBolsillo, String celular, double saldo) {
        
        Bolsillo bolsillo = new Bolsillo();
        int filasActualizadas = bolsillo.retirarDineroBolsillo(nombreBolsillo, celular, saldo);
